...
class ICT02 extends StatefulWidget {
  const ICT02({Key? key}) : super(key: key);

  @override
  _ICT02State createState() => _ICT02State();
}

class _ICT02State extends State<ICT02> {
  ...
  void initState() {
    ...
    SharedPreferences.getInstance().then((prefs) {
      ...
      Future.delayed(Duration.zero, () {
        setState(() {
          futureStudents = fetchStudents('IT', '2', token);
        });
      });
    });
  }
  ...
  Future<void> deleteStudent(int id) async {
    try {
      ...
      setState(() {
        futureStudents = fetchStudents('ICT', '2', token);
      });
    } catch (error) {
